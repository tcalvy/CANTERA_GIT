...
pattern = re.compile(r'diffusion_flame_H2AIR_(\d+)p(\d+)_(\d+)p(\d+)\.csv')
...
# PROCESS FILES
# ----------------------------------------------------------
for filepath in glob.glob(os.path.join(folder, "diffusion_flame_H2AIR_*.csv")):
...
plt.grid(True)
plt.tight_layout()
plt.savefig(os.path.join(output_folder, "XCO_grid.png"), dpi=300)
plt.show()
...
plt.title("Position of Maximum OH vs Mass Flow Rate Ratio")
plt.savefig(os.path.join(output_folder, "max_XCO_ratio_mf.png"), dpi=300)
plt.grid(True)
plt.tight_layout()
plt.show()
